    ...
    <section class="assessment">
            ...
            <div class="progress-info">
                <span class="progress-text">
                    {{ $t("MarketingAssessment.progressText", {
                        completed: completedQuestions, total: questions.length
                    }) }}
                </span>
                <span class="progress-percentage">
                    {{ $t("MarketingAssessment.progressPercentage", { percentage: Math.round(progressPercentage) }) }}
                </span>
            </div>
        ...
        <form @submit.prevent="submitAnswers">
            <transition name="fade" mode="out-in">
                <div v-if="currentQuestion" :key="currentQuestionIndex" class="question-card">
                    <span class="question-number">
                        {{ $t("MarketingAssessment.questionNumber", { number: currentQuestionIndex + 1 }) }}
                    </span>
                    <h3 class="question-text">{{ currentQuestion.question }}</h3>

                    <div class="answers-container">
                        <label v-for="(answer, index) in currentQuestion.answers" :key="index" class="answer-option"
            ...
            <div class="navigation">
                <button type="button" class="nav-button prev-button" @click="prevQuestion"
                    :disabled="currentQuestionIndex === 0">
                    {{ $t("MarketingAssessment.prevButton") }}
                </button>

                <button v-if="currentQuestionIndex < questions.length - 1" type="button" class="nav-button next-button"
                    @click="nextQuestion" :disabled="answers[currentQuestionIndex] === undefined">
                    {{ $t("MarketingAssessment.nextButton") }}
                </button>

                <button v-else type="submit" class="nav-button submit-button" :disabled="!allQuestionsAnswered">
...
export default {
    ...
    computed: {
        questions() {
            const locale = this.$i18n.locale; // Get the current locale (e.g., 'en' or 'vi')
            const questions = this.$i18n.messages[locale].MarketingAssessment.questions; // Access the questions array
            return questions;
        },
        completedQuestions() {
    ...
    methods: {
        async submitAnswers() {
            try {
                ...
                const response = await axios.post(`${baseUrl}/api/questionnaire/submit`, {
